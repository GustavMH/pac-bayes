...
def test_training():
    a = np.load(Path("../../oracle-bounds-2/mvb.npz"))
    b = gen_rf_risks(100,1)['SVMGuide1'][0]
    gibbs_diff = (b["gibbs_risks"] - a["gibbs_risks"]).sum()
    tandem_diff = (b["tandem_risks"] - a["tandem_risks"]).sum()
    print(gibbs_diff, tandem_diff)
